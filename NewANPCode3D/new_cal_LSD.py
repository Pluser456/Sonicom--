...
def evaluate_one_hrtf(model, test_loader, use_diff=False):
    model.eval()
...
if __name__ == "__main__":
    ...
    for hrtfid in range(1, len(right_test)+1):  # 选择计算第几个HRTF的LSD
        ...
        pred_log_hrtf, true_log_hrtf = evaluate_one_hrtf(model, dataloader, use_diff=use_diff)
    ...
    np.savetxt(f'{path}\\lsd_CNN_{input_type}_Wi.txt', avg_lsd_per_freq, fmt='%.3f', header='LSD (dB)')