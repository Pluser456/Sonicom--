...
def evaluate_one_hrtf(model, test_loader, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            meanloghrtf = meanloghrtf[:, :, :]  # 形状变为 [1, 793]
            meanloghrtf = meanloghrtf.squeeze(0).to(device) 
...
def main(args):
    ...
    res_list_mean = []
    # 将均值转为tensor
    log_mean_hrtf_right = torch.tensor(np.abs(log_mean_hrtf_right[:, target_index]), dtype=torch.float32).to(device)
...
if __name__ == '__main__':
    ...
    for target_index in range(Freq_Num):
        ...
        lsd_predicted.append(predicted**2)
        lsd_mean.append(mean**2)
        
    print(f"平均预测LSD{np.sqrt(np.mean(lsd_predicted))}")
    print(f"平均LSD{np.sqrt(np.mean(lsd_mean))}")
