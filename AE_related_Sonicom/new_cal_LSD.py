...
if __name__ == "__main__":
    ...
    # 保存频率数据
    path = f'HRTF可视化'
    np.savetxt(f'{path}\\freq_data_Sonicom.txt', freq_list, fmt='%.1f', header='Frequency (Hz)')

    # 保存LSD数据
    np.savetxt(f'{path}\\lsd_data_Sonicom.txt', avg_lsd_per_freq, fmt='%.3f', header='LSD (dB)')
    np.savetxt(f'{path}\\lsd_mean_data_Sonicom.txt', avg_lsd_per_freq_of_mean, fmt='%.3f', header='LSD (dB)')
