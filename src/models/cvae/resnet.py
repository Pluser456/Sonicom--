...
#这个代码似乎是被放弃的，不如ConvVAE完善，要修改参考vae_conv_cfg文件
...
class ResNetVAECfg(VAE):
    ...
    def configure_optimizers(self):#此处代码同ConvVAE
    ...
    @staticmethod
    def add_model_specific_args(parent_parser):#此处代码同ConvVAE
        parser = ArgumentParser(parents=[parent_parser], add_help=False)
        parser.add_argument('--model_cfg_path', type=str, default='./configs/models/vae_resnet/18.json')
        return parser

    def training_epoch_end(self, outputs):#此处代码同ConvVAE，但不如其成熟
        # log gradients
    ...
    def get_pred_ear_figure(self, ear_true, labels, n_images=6):#此处代码原理同ConvVAE
