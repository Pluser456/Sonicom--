...
VAE 训练脚本 - 支持配置文件
用法:
    python train_vae.py --config configs/vae/default.yaml
"""
import os
import argparse
...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='configs/vae-dnn-cvae/vae_default.yaml',
...
def loss_function(ear_true, ear_pred, means, log_var, kl_coeff):
    """
    VAE 损失函数
    Args:
        ear_true: 真实耳朵图像 [batch_size, 1, H, W]
        ear_pred: 重构耳朵图像 [batch_size, 1, H, W]
    ...
    Returns:
        ...
        loss: 总损失
    """
    mse = nn.functional.mse_loss(ear_pred, ear_true, reduction='sum') / ear_true.size(0)
    kld = -0.5 * torch.sum(1 + log_var - means.pow(2) - log_var.exp()) * kl_coeff / ear_true.size(0)
    loss = mse + kld
    return mse, kld, loss


def get_pred_ear_figure(ear_true, ear_pred, n_cols=4):
    """
    将真实和重构的耳朵图像拼接成一个大图用于可视化
    Args:
        ear_true: 真实图像 [batch_size, 1, H, W]
        ear_pred: 重构图像 [batch_size, 1, H, W]
        n_cols: 每行显示的样本数
    Returns:
        拼接后的图像张量 [1, H*2, W*n_cols]
    """
    bs = ear_true.shape[0]
    n_rows = max(bs, 1)
    # 确保在 CPU 上
    ear_true = ear_true.cpu()
    ear_pred = ear_pred.cpu()

    # 归一化到 [0, 1] 用于显示
    ear_true = (ear_true - ear_true.min()) / (ear_true.max() - ear_true.min() + 1e-8)
    ear_pred = (ear_pred - ear_pred.min()) / (ear_pred.max() - ear_pred.min() + 1e-8)

    rows = []
    for i in range(min(bs, n_cols)):
        # 垂直拼接真实和预测图像
        pair = torch.cat([ear_true[i, 0], ear_pred[i, 0]], dim=0)
        rows.append(pair)
...
def main():
    ...
    model = VAE(
        ...
        use_inception=config.model.use_inception,
        repeat_per_block=config.model.repeat_per_block
    ).to(device)
    ...
    # 使用 ReduceLROnPlateau 或者 cosine schedule
    ...
    kl_coeff = config.model.kl_coeff
    ...
    # 确定使用哪个图像字段
    image_field = "left_voxel" if config.dataset.mode == "left" else "right_voxel"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, batch in enumerate(train_progress_bar):
            ear_img = batch[image_field].to(device)

            optimizer.zero_grad()

            reconstructed_ear, mu, logvar = model(ear_img)
            recon_loss, kl_loss, total_loss = loss_function(ear_img, reconstructed_ear, mu, logvar, kl_coeff)
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(val_progress_bar):
                ear_val = batch[image_field].to(device)

                reconstructed_ear_val, mu_val, logvar_val = model(ear_val)
                recon_loss_val, kl_loss_val, _ = loss_function(ear_val, reconstructed_ear_val, mu_val, logvar_val, kl_coeff)

                val_loss_recon += recon_loss_val.item()
                val_loss_kl += kl_loss_val.item()

                # 只记录第一个 batch 的图像
                # if writer is not val_images_logged:
                #     fig = get_pred_ear_figure(ear_val, reconstructed_ear_val)
                #     writer.add_image("val/reconstruction", fig, epoch)
