...
class ResNet2DClassifier(nn.Module):
    ...
    def __init__(self, d_model=16, encoder_out_vec_num=8):
        super(ResNet2DClassifier, self).__init__()
        self.feature_extractor = resnet18(num_classes=384)
        self.encoder_out_vec_num = encoder_out_vec_num
        self.d_model = d_model
        self.vq_layer = nn.ModuleList([ nn.Sequential(nn.Linear(384, d_model),
            nn.Dropout(p=0.3),
            nn.ReLU(), nn.Linear(d_model, d_model)) for _ in range(encoder_out_vec_num)])

    def forward(self, right_voxel, device):
        # 体素特征提取
        right_voxel = right_voxel.to(device)
        features = self.feature_extractor(right_voxel) # [batch_size, img_feature_dim]
        predictions = torch.zeros((features.shape[0], self.encoder_out_vec_num, self.d_model)).to(device)  # [batch_size, encoder_out_vec_num, d_model]
        for i in range(self.encoder_out_vec_num):
            predictions[:, i, :] = self.vq_layer[i](features)
...
class ResNet3DClassifier(nn.Module):
    """修改为多头分类网络，每个位置使用独立子头，基于3D特征提取器"""
    modelname = "3DResNetClassifier"
    def __init__(self, d_model=16, encoder_out_vec_num=8):
        super(ResNet3DClassifier, self).__init__()
        self.feature_extractor = resnet18_3d(num_classes=384)
        self.encoder_out_vec_num = encoder_out_vec_num
        self.d_model = d_model
        self.vq_layer = nn.ModuleList([ nn.Sequential(nn.Linear(384, d_model),
            nn.Dropout(p=0.3),
